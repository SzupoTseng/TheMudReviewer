...
def title_for(base, text):
    # 本书与《开源器识》的差别：一个「篇」跨多个档（例如第六篇有四档），
    # 所以 H1 是「篇名」而非章名，章名改由文件名 + 单元编号范围组出。
...
def part_for(text):
    # 有「# 第N篇 …」H1 的档＝该篇的第一档，在 nav 插一个分组标题。
...
def build_merged():
    ...
    parts.append("> 全书合并版。本书为教育与研究用途：把一套没有规格书的 MUD 协议还原出来、再据以重写一个客户端，"
                 "并完整记录七次交付了不能用的东西。客户端为净室实作，未拷贝任何原代码或资源。\n")
    parts.append("\n---\n\n## 目录\n")
    for base in FILES:
...
def build_html():
    ...
    page = f"""<!doctype html><html lang="zh-Hans"><head><meta charset="utf-8"/>
