...
# 在地化：簡體版的 HTML 語言標記要是 zh-Hans（opencc 不會轉這個）
bcn = bcn.replace('zh-Hant', 'zh-Hans')
with open(os.path.join(OUT, '_build.py'), 'w', encoding='utf-8') as w:
    w.write(bcn)
print('_build.py -> cn/_build.py')

# 複製封面到 cn/（讓 cn 構建與根目錄一致地找到封面）
for cov in ('TheMudReviewer.png', 'cover.png'):
    src = os.path.join(BOOK, cov)
    dst = os.path.join(OUT, cov)
    if os.path.exists(src):
        # ★ 封面是圖，opencc 轉不了裡面的字。若 cn/ 已經有一張「簡體版封面」，
        #   就不要用繁體版覆蓋它 —— 否則每次轉換都會把在地化封面洗掉。
        if os.path.exists(dst):
            print('cover 保留既有簡體版:', os.path.join('cn', cov))
        else:
            shutil.copy(src, dst)
