...
# -*- coding: utf-8 -*-
"""組裝《泥巴考古學》成品:
  1) 合併單一 Markdown 檔  TheMudReviewer.md
  2) 圖文 HTML(側邊子目錄)  TheMudReviewer.html
版面沿用《開源器識》_build.py(同一套 CSS / 解析器 / 封面邏輯),
依 BUILD_STANDARD.md：只改常數區與 title_for/part_for 在地化 regex。
內容為一套沒有規格書的 MUD 協議的逆向、重寫與七次事故。
...
BOOK_TITLE = "泥巴考古學"          # 顯示用（<h1>／<title>／封面 alt）
BOOK_SLUG  = "TheMudReviewer"   # 檔名用（純 ASCII，避免網址出現 percent-encoding）
BOOK_SUB   = "從一個 APK 到三十七年的驅動血脈 · The MUD Reviewer"
COVER_CANDIDATES = ["TheMudReviewer.png", "泥巴考古學.png", "cover.png"]
...
def title_for(base, text):
    # 本書與《開源器識》的差別：一個「篇」跨多個檔（例如第六篇有四檔），
    # 所以 H1 是「篇名」而非章名，章名改由檔名 + 單元編號範圍組出。
    if base.startswith("00_"):
        return "序言"
    if base.startswith("01_"):
        return "導讀 · 證據等級的判讀學"
    if base.startswith("附錄"):
        m = re.search(r'^#\s+(.*)$', text, re.M)
        return m.group(1).strip() if m else base
    stem = re.sub(r'^\d{2}_', '', base[:-3]).replace('_', ' · ')
    units = re.findall(r'^##\s+(\d{3})', text, re.M)
    if units:
        rng = units[0] if len(units) == 1 else f"{units[0]}–{units[-1]}"
        return f"{stem}（{rng}）"
    return stem

def part_for(text):
    # 有「# 第N篇 …」H1 的檔＝該篇的第一檔，在 nav 插一個分組標題。
    m = re.search(r'^#\s+(第.{1,3}篇.*)$', text, re.M)
...
def build_merged():
    ...
    parts.append("> 全書合併版。本書為教育與研究用途：把一套沒有規格書的 MUD 協議還原出來、再據以重寫一個客戶端，"
                 "並完整記錄七次交付了不能用的東西。客戶端為淨室實作，未複製任何原始碼或資源。\n")
    parts.append("\n---\n\n## 目錄\n")
    for base in FILES:
